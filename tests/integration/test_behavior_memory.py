...
def test_add_and_len_and_file_creation(scratch_dir: str, dummy_rollout: ROLLOUT):
    ...
    bm.add(h0, dummy_rollout, obs, acts)
...
@pytest.mark.memory
def test_replacement_of_worse_rollout(scratch_dir: str, dummy_rollout: ROLLOUT):
    bm = BehaviorMemory(scratch_dir, max_size=1)
    obs, acts = make_obs_act()
    h0 = torch.zeros(1, 16)
    
    better = (dummy_rollout[0], dummy_rollout[1], dummy_rollout[2], 1.5)			# Add first rollout with reward=1.5
    bm.add(h0, better, obs, acts)
    
    worse = (dummy_rollout[0], dummy_rollout[1], dummy_rollout[2], 1.0)				# Add a worse rollout for same key
    bm.add(h0, worse, obs, acts)
...
def test_sample_and_max_size(scratch_dir: str, dummy_rollout: ROLLOUT):
    ...
    for i in range(2):								# Add two distinct keys
        h = torch.ones(1, 16) * i
        bm.add(h, dummy_rollout, obs, acts)
...
def test_recompute_keys(scratch_dir: str, dummy_rollout: ROLLOUT):
	...
	h1 = torch.ones(1, 16)
	bm.add(h0, dummy_rollout, obs, acts)
	bm.add(h1, dummy_rollout, obs, acts)
